...
Necesitas Python 3.11 o superior y las librerías de Pango (lo que usa WeasyPrint para maquetar):
...
|---|---|---|
| `POST` | `/api/convert/pdf-to-md` | `file` (PDF). `output` = `json` \| `md` \| `zip`. `images` = `embed` \| `none`. |
...
4. **OCR** para PDF escaneados (Tesseract u otro servicio), como función del plan Pro.
5. **Páginas legales**: términos de uso, política de privacidad (RGPD) y aviso de cookies.

## Licencias

Todas las dependencias tienen licencias permisivas que permiten el uso comercial sin publicar tu código: FastAPI (MIT), pdfplumber (MIT), pdfminer.six (MIT), pypdf (BSD), pypdfium2 (Apache-2.0/BSD), WeasyPrint (BSD), markdown-it-py (MIT) y Pygments (BSD). Se evitó a propósito PyMuPDF, cuya licencia AGPL obligaría a publicar el código del servicio o a comprar una licencia comercial.
...
- Los PDF escaneados (imágenes sin texto) no se pueden convertir a texto sin OCR; en su lugar se extraen como imágenes y se muestra un aviso.
...
- Las fórmulas matemáticas (LaTeX) aún no se admiten en Markdown → PDF.
